...
void process_request(connection* conn) {
    ...
    /* Read entire HTTP request into request buffer one line at a time */
    ...
    while (1) {
        ...
        /* If not more room in the request buffer, make room */
    ...
    while( (n = Rio_readn_w(serverfd, buf, MAXLINE)) > 0 ) {
        response_len += n;
        Rio_writen_w(conn->connfd, buf, n);
        bzero(buf, MAXLINE);
    }
     ...
     */
    format_log_entry(log_entry, &conn->clientaddr, request_uri, response_len);  
